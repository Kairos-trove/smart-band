...
void DS1302_WriteRam(uint8_t address, uint8_t data)
{
	if (address > 30)
	{
		return;
	...
	DS1302_WriteReg((uint8_t)(0xC0 + address * 2), data);
}

uint8_t DS1302_ReadRam(uint8_t address)
{
	if (address > 30)
	{
		return 0;
	...
	return DS1302_ReadReg((uint8_t)(0xC0 + address * 2));
}
